...
module "network_interface_sli" {
  ...
  aws_interface_subnet_id         = var.aws_existing_sli_subnet_id != null ? var.aws_existing_sli_subnet_id : aws_subnet.sli.0.id
  ...
  custom_tags                     = merge(var.common_tags, {
    ...
    "ves-io-eni-az"         = var.aws_existing_sli_subnet_id != null ? var.aws_existing_sli_subnet_id : aws_subnet.sli.0.availability_zone
  })
}
...
resource "aws_route_table_association" "subnet_sli" {
  count          = var.is_multi_nic ? 1 : 0
  subnet_id      = var.aws_existing_sli_subnet_id != null ? var.aws_existing_sli_subnet_id : aws_subnet.sli.0.id
